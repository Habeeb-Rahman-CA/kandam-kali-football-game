...
func _ready() -> void:
	...
	GameEvents.team_scored.connect(on_team_reset.bind())
...
func spawns_players(club: String, own_goal: Goal) -> Array[Player]:
	var player_nodes : Array[Player] = [null]
	var players := DataLoader.get_squad(club)
	...
	for i in players.size():
		...
		var player := spawn_player(player_position, kickoff_position, own_goal, target_goal, player_data, club)
		player_nodes.append(player)
		add_child(player)
	return player_nodes
	
func spawn_player(player_position: Vector2, kickoff_position: Vector2, own_goal: Goal, target_goal: Goal, player_data: PlayerResource, club: String) -> Player:
	var player : Player = PLAYER_PREFAB.instantiate()
	player.initialize(player_position, kickoff_position, ball, own_goal, target_goal, player_data, club)
...
func set_on_duty_weights() -> void:
	for squad in [squad_away, squad_home]:
		var cpu_players : Array[Player] = squad.filter(
			func(p: Player): return p != null and p.control_scheme == Player.ControlScheme.CPU and p.role != Player.Role.GOALIE
		)
...
func on_player_swap_request(requester: Player) -> void:
	var squad := squad_home if requester.club == squad_home[0].club else squad_away
	var cpu_players : Array[Player] = squad.filter(
		func(p: Player): return p != null and p.control_scheme == Player.ControlScheme.CPU and p.role != Player.Role.GOALIE
	)
	cpu_players.sort_custom(func(p1: Player, p2: Player):
...
func setup_control_schemes() -> void:
	var p1_club := GameManager.player_setup[0]
	if GameManager.is_coop():
		var player_squad := squad_home if squad_home[0].club == p1_club else squad_away
		...
		var p1_squad := squad_home if squad_home[0].club == p1_club else squad_away
