...
func spawns_players(club: String, own_goal: Goal) -> Array[Player]:
	...
	for i in players.size():
		...
		var player := spawn_player(player_position, own_goal, target_goal, player_data, club)
		player_nodes.append(player)
		add_child(player)
	return player_nodes
	
func spawn_player(player_position: Vector2, own_goal: Goal, target_goal: Goal, player_data: PlayerResource, club: String) -> Player:
	var player : Player = PLAYER_PREFAB.instantiate()
	player.initialize(player_position, ball, own_goal, target_goal, player_data, club)
