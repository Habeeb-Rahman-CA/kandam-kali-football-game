...
func perform_ai_movements() -> void:
	...
	if player.has_ball():
		...
		if is_ball_carried_by_teammate():
			total_steering_force += get_assist_formation_steeting()
...
func perform_ai_decisions() -> void:
	...
	if ball.carrier == player:
		...
		elif has_opponent_nearby() and randf() < PASS_PROBABILITY:
...
func get_assist_formation_steeting() -> Vector2:
	var spawn_difference := ball.carrier.spawn_position - player.spawn_position
	var assist_destination := ball.carrier.position - spawn_difference * SPREAD_ASSIT_FACTOR
...
func is_ball_possessed_by_opponent() -> bool:
	return ball.carrier != null and ball.carrier.club != player.club
...
func is_ball_carried_by_teammate() -> bool:
	return ball.carrier != null and ball.carrier != player and ball.carrier.club == player.club

func has_opponent_nearby() -> bool:
	var players := opponent_detection_area.get_overlapping_bodies()
	return players.find_custom(func(p: Player): return p.club != player.club) > -1
