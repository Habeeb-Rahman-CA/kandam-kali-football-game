...
func initializer(context_club: String) -> void:
	club = context_club
...
func on_ball_enter_scoring_area(_ball: Ball) -> void:
	GameEvents.team_scored.emit(club)
...
func get_center_target_position() -> Vector2:
	return targets.get_child(int(targets.get_child_count() / 2)).global_position

func get_top_target_position() -> Vector2:
	return targets.get_child(0).global_position

func get_botton_target_position() -> Vector2:
